...
int direction = 1; // 1 = move from 0 to 180, -1 = move from 180 to 0
...
void checkInputAndRotate() {
	if(pos >= MAX_ROTATE_ANGLE) {
		pos = MAX_ROTATE_ANGLE;
		direction = -1;
	}

	if(pos <= MIN_ROTATE_ANGLE) {
		pos = MIN_ROTATE_ANGLE;
		direction = +1;
	}
...
void loop() {
	...
	// if(CONTROL_USING_BUTTONS && touchRead(4) <= 10) {
